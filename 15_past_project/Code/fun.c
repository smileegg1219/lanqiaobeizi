...
uint8_t flag_K_A, flag_K_B;
uint16_t para_PD = 1000;
uint16_t para_PH = 5000;
int16_t para_PX;
uint8_t para_indax; // para
uint8_t data_flag;
uint32_t capture_valueA;
uint32_t capture_valueB;
uint8_t math_flag_A, math_flag_B;
uint8_t lcd_mode;
uint8_t NDA,NDB,NHA,NHB;
uint16_t count_3s;
int16_t free_A_max, free_B_max;
int16_t free_A_min = 25000;
int16_t free_B_min = 25000;
uint8_t flag_long;
uint8_t B1_data,B2_data,B3_data,B4_data,B1_last,B2_last,B4_last;
uint8_t B3_last = 1;
uint16_t count_1s;
void key_scan(void)
{
	...
	if(lcd_mode != 2)
	{
		if(!B3_data&B3_last) // B3
		{
			...
			else if(lcd_mode == 0) data_flag ^= 1;
		}
	}
	else
	{
	...
	switch(lcd_mode)
	{
		case 0:
			...
			if(!math_flag_A)
			{
				if(!data_flag)
				{
					if(!flag_K_A) sprintf(text, "     A=%dHz         ", free_A);
					else sprintf(text, "     A=%.2fKHz        ", (float)free_A/1000);
				}
				else
				{
					if(!flag_K_A) sprintf(text, "     A=%duS         ", capture_valueA);
					else sprintf(text, "     A=%.2fmS         ", (float)capture_valueA/1000);
				}
			}
			...
			if(!math_flag_B)
			{
				if(!data_flag)
				{
					if(!flag_K_B) sprintf(text, "     B=%dHz         ", free_B);
					else sprintf(text, "     B=%.2fKHz        ", (float)free_B/1000);
				}
				else
				{
					if(!flag_K_B) sprintf(text, "     B=%duS         ", capture_valueB);
					else sprintf(text, "     B=%.2fmS         ", (float)capture_valueB/1000);
				}
			}
...
{
	//PX
	if(free_A<0) math_flag_A = 1;
	else math_flag_A = 0;
	if(free_B<0) math_flag_B = 1;
	else math_flag_B = 0;
	capture_valueB  = 1000000/free_B;
	capture_valueA  = 1000000/free_A;
	if(!data_flag)
	{
		if(free_A>1000) flag_K_A = 1;
		else flag_K_A = 0;
		if(free_B>1000) flag_K_B = 1;
		else flag_K_B = 0;
	}
	else
	{
		if(capture_valueA>1000) flag_K_A = 1;
		else flag_K_A = 0;
		if(capture_valueB>1000) flag_K_B = 1;
		else flag_K_B = 0;
	}
	//PH
	if(!PH_flag_B)
	...
	if(free_A_max<free_A) free_A_max = free_A;
	if(free_A_min>free_A) free_A_min = free_A;
	if(free_B_max<free_B) free_B_max = free_B;
	if(free_B_min>free_B) free_B_min = free_B;
}
void main_pros(void)
{
...
{
	if(htim->Instance == TIM16) // B
	{
		uint32_t value_cap = TIM16->CCR1 + 1;
		TIM16->CNT = 0;
		free_B = 1000000/value_cap;
		if(free_B>20000) free_B = 20000;
		else if(free_B<400) free_B = 400;
		free_B+=para_PX;
	}
	if(htim->Instance == TIM2) // A
	{
		uint32_t value_cap = TIM2->CCR1 + 1;
		TIM2->CNT = 0;
		free_A = 1000000/value_cap;
